...
class Satem:
    def __init__(self):
        ...
        # Input for sql query  (specific to obstool  & satem  diagnostics !)
...
class Conv:
    def __init__(self ):
        ...
        # ONLY FOR CONV OBS 
        # Input for sql query  (specific to obstool diagnostics & conv !)
...
class ExtractData:
    def __init__(self , dbpath , obs_kind  ):
       ...
       #if self.obs_kind  != 'conv'  or self.obs_kind != 'satem':
    ...
    def get_odb_rows  (self,  bdate , edate , obs_list,
        ...
        period = self.st.set_period( bdate , edate  )
        types  = ObsType ()
        sat_list, _ , _ ,sens_name_dict, sensor_dict = types.SatDict()

        if reprocess_odb   ==True:              # Extract rows again if new obs is added for example  !
    ...
    def dist_matrix  (self , bdate , edate , obs_list):
        
        # Gather all small chunks in  *.tmp files 
        period = self.st.set_period( bdate , edate  )
        _, varobs = self.st.set_obs_list ( obs_list  , self.obs_kind  )
...
class Setting:
    ...
    """def update_conv_list(self, list_):
        types             = ObsType ()
        obs_dict ,  _     = types.ConvDict()
        keys  = []
        values= []
    ...
    def update_satem_list(self, list_):
        types             = ObsType ()
        if   obs_kind == 'conv':
            obs_dict ,  _     = types.ConvDict()
